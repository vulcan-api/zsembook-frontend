...
const DeleteModal = (props: any) => {
  ...
  useEffect(() => {
    const getRemovablePost = async () => {
      await fetch(`http://${process.env.REACT_APP_REQUEST_URL}/spotted/post/${props.postId}`, {
        method: "GET",
        credentials: "include",
      })
        .then((res) => res.json())
    ...
    const getRemovableProject = async () => {
      await fetch(`http://${process.env.REACT_APP_REQUEST_URL}/project/${props.projectId}`, {
        method: "GET",
        credentials: "include",
      })
        .then((res) => res.json())
  ...
  const deleteProject = async (projectId: number) => {
    const response = await fetch(`http://${process.env.REACT_APP_REQUEST_URL}/project/${projectId}`, {
      method: "DELETE",
      credentials: "include",
    });
    if (response.ok) {
  ...
  const deletePost = async (postId: number) => {
    const response = await fetch(
      `http://${process.env.REACT_APP_REQUEST_URL}/spotted/post/${postId}`,
      {
        method: "DELETE",
        credentials: "include",
      }
